// データアクセス層。
// 画面から使うデータ操作を、ここ1か所に集約する。
// UI は必ずこの層を経由し、IndexedDB を直接触らない。
// 保存先は IndexedDB だけで、外へ送る仕組みは持たない。
 ...
 * その日の予定の版（revision）。
 *
 * 自動スケジューラは「作った案を当てるまでに、その日の予定が変わっていないか」を
 * ここで見る。学習中に予定が動いても、古い案で上書きしてしまわないための番号である。
 */
...
/** その日の予定の版を1つ進める。予定を書き換えたときに必ず通る。 */
export async function bumpPlanRevision(date, { updatedBy = 'app' } = {}) {
  const all = await getPlanMeta();
  const current = all[date] ?? { revision: 0 };
  return setPlanMeta(date, {
...
export async function updateTodayTasks(tasks, date = studyDayKey(), { bumpRevision = true, updatedBy = 'app' } = {}) {
  const now = new Date().toISOString();
  const normalized = tasks.map((t, i) => withItems({
    // IDは渡されたものを必ず残す。作り直すと、実績や繰り越しの結び付きが切れてしまう。
    id: t.id || uid('task'),
    date,
  ...
  // 予定を書き換えたので版を1つ進める。作りかけの自動計画が古くなったことが分かる。
  if (bumpRevision) await bumpPlanRevision(date, { updatedBy });
  return normalized;
}
...
export async function undoStudyRecord(recordId) {
  return idb.undoAttempt(recordId, (record, records, tasks) => {
    const changed = [];
...
export async function purgeStudyData() {
  ...
  for (const store of [STORES.records, STORES.challenges, STORES.tasks, STORES.goals, STORES.moves]) {
    await idb.clear(store);
  }
  // 予定の版・学習可能時間・見積もりの指定も、いっしょに初期状態へ戻す。
  // 表示の設定は消さない。消すと使えなくなってしまうため。
...
/**
 * 問題別の見積もり指定（本人が決めた時間と、教材から入れた仮の値）。
